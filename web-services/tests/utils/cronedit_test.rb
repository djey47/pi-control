...
class CronEdit_test < Test::Unit::TestCase
  ...
  def teardown
    ...
    def test_complex
        Crontab.publicize_methods {
            File.open( File.join(File.dirname(__FILE__), '../resources/testcron.txt') ) { |f|
                content = f.read
                f.rewind
...
##        puts "--------------entries:  \n#{entries.map {|k,v| "#{k}=>#{v}\n"} }"
    ...
    def test_filecrontab
        fc = FileCrontab.new File.join(File.dirname(__FILE__), '../resources/testcron.txt'), '/tmp/crontest.txt'
    ...
    def test_mergeWithFile
        fc1 = FileCrontab.new File.join(File.dirname(__FILE__), '../resources/testcron.txt')
...
##        puts "\n\n"+output
    ...
    def test_subtractFile
        ...
        fc2= FileCrontab.new File.join(File.dirname(__FILE__), '../resources/testcron.txt')
        fc1.subtract fc2
        fc1.commit
        output = fc1.to_s
##        puts "\n\n"+output
